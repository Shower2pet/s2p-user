...
-- Update auto_offline threshold to 3 minutes
...
-- Update get_public_stations heartbeat check to 3 minutes
...
AS $$
  SELECT
    s.id, s.type,
    CASE
      WHEN s.status::text = 'AVAILABLE' AND s.last_heartbeat_at < now() - interval '3 minutes' THEN 'OFFLINE'
